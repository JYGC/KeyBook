<script lang="ts">
	...
	let { registerAndLogin, registerModule = $bindable() } = $props<{
...
<FluidForm>
	...
	<PasswordInput
		...
		bind:value={registerModule.password}
	/>
	<PasswordInput
		labelText="Confirm password"
		placeholder="Confirm password..."
		bind:value={registerModule.passwordConfirm}
	/>
</FluidForm>
<br />
<Button
	disabled={registerModule.email.length === 0 ||
		registerModule.password.length < 8 ||
		registerModule.passwordConfirm !== registerModule.password}
	onclick={registerAndLogin}>Create account!</Button
>
