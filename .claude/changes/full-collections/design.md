...
Every other place `cobrandAdmins_via_cobrand.user.id = @request.auth.id` (at any relation depth) authorizes a create, update, or delete elsewhere in section 2.3 is rewritten as `(...cobrandAdmins_via_cobrand.user.id ?= @request.auth.id && ...cobrandAdmins_via_cobrand.approved ?= true)`.

**The `?=` operator is required, not optional.** PocketBase's plain `=` operator on a to-many back-relation (`cobrandAdmins_via_cobrand`) is not "at least one related row matches" — it compiles to a "multi-match" guard requiring *every* related row to satisfy the condition, expressed as `(row matches) AND NOT EXISTS (a related row that doesn't match)`. The moment a cobrand has two admins (exactly what this feature introduces — an approved admin alongside an unapproved one), a plain `=` check on `cobrandAdmins_via_cobrand.user.id` fails for *everyone*, because there is always at least one related row (the other admin) that isn't `@request.auth.id`. This broke every rule referencing the relation, including read-only list/view rules whose business logic never changed — those needed the same `=` → `?=` fix even though they aren't part of the approval gate itself. `?=` (`fexpr.SignAnyEq`) disables that guard (`isAnyMatchOp` skips the wrapping in `tools/search/filter.go`), restoring ordinary "at least one row" semantics. Verified empirically (contract tests, plus manual reproduction against a scratch PocketBase instance) that two `?=` conditions sharing the same `..._via_cobrand` prefix still correlate to the *same* joined row — PocketBase's field resolver reuses one SQL join per unique relation path, so `.user.id` and `.approved` conditions on the same path share the same table alias. An unapproved admin cannot piggyback on a different, approved co-admin's row.

`approved` can only be changed by a KeyBook staff member with PocketBase superuser access — the `cobrandAdmins` update rule is locked (`null`) rather than left open to existing admins, since that's the only field on the record worth changing after creation and it must not be self-service. Approving a cobrand is therefore an out-of-band support action, not an app feature. A `null` rule rejects with 403 ("Only admins can perform this action") rather than the usual 404 a filter-rule rejection produces — this affects the expected status code for any existing test touching `cobrandAdmins` update.

`items`, `entryDevices`, and `personItems` have no cobrand-owner/manager clause to gate at all: their real update/delete/CUD rules only ever enforce the person-owner clause, because the cobrand-owner/manager check would require a relation chain deeper than PocketBase v0.22 supports (see the "PocketBase v0.22 limitation" comments already in `1748563200_full_collections.go`). Section 2.3's description of those three collections' rules is the actual implemented (narrower) form, not the fuller form implied elsewhere in this document — the approval gate has nothing to add there.
...
**Where the "inviting admin must be approved" check lives:** not in a raw access rule (PocketBase's declarative rules can't express "the *other* related row must not exist" bootstrap logic any more cleanly here than the existing bootstrap cases in this section already require hook/application-service enforcement) and not exercised by this project's HTTP contract-test harness — `newTestEnv()` (`backend/contract/setup_test.go`) builds its PocketBase app via `tests.NewTestApp` + `apis.InitApi` directly and never calls `cmd.startBackend()`, so *none* of `keybook.go`'s custom hooks run under it, including the pre-existing duplicate-admin check this same hook already performed. The check lives in `CobrandApplicationService.AddCobrandAdmin` (new `inviterUserId` parameter, enforced via `CobrandService.EnsureInviterIsApprovedAdmin`) and is covered by an `internal/application` test that calls the service directly; the same logic is also invoked from the `cobrandAdmins` create hook in `cmd/keybook.go` for the real HTTP request path the frontend actually uses.
