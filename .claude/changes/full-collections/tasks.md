...
- [ ] **1.3** In the same migration, set PocketBase access rules for all collections using the new ownership chain (`propertyOwners → personPropertyOwners → person.user` and `cobrandPropertyOwners → cobrand → cobrandAdmins.user`). For `persons`: list/view = union of own-user, tenant-property-owner/manager, and item-property-owner/manager checks; create = `@request.auth.id != ""`; update/delete = `user.id = @request.auth.id`. For `properties`: list/view = union of person-owner, cobrand-owner, cobrand-manager, tenant, household-member, and agent checks; create = `@request.auth.id != ""`; update/delete = person-owner or cobrand-owner only. For `propertyOwners`: list/view = person-owner, cobrand-owner, or cobrand-manager of the linked property; create = `@request.auth.id != ""` (bootstrap and existing-owner enforcement in `PropertyOwnerApplicationService`); update/delete = person-owner or cobrand-owner of the linked property; last-owner-deletion guard in the application service. For `personPropertyOwners`: list/view = person-owner, cobrand-owner, or cobrand-manager of the linked property (routed via `propertyOwner.property`); create/update/delete = person-owner or cobrand-owner only (managers cannot modify ownership sub-records). For `cobrandPropertyOwners`: list/view = same three groups via `propertyOwner.property`; create/update = person-owner or cobrand-owner only; delete = person-owner, cobrand-owner, OR admin of the cobrand being removed (resignation). For `cobrandPropertyManagers`: list/view = person-owner, cobrand-owner, OR admin of the managing cobrand; create/update = person-owner or cobrand-owner only; delete = person-owner, cobrand-owner, OR admin of the managing cobrand (resignation). For `cobrands`: list/view = cobrand admin or agent of that cobrand; create = `@request.auth.id != ""`; update/delete = cobrand admin only. Full filter expressions in `design.md` section 2.3. Confirm contract tests from 1.1 pass.
