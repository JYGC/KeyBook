...
- [ ] **1.3** In the same migration, set PocketBase access rules for all collections using the new ownership chain (`propertyOwners → personPropertyOwners → person.user` and `cobrandPropertyOwners → cobrand → cobrandAdmins.user`). For `persons` specifically: list/view = union of own-user, tenant-property-owner/manager, and item-property-owner/manager checks (see `design.md` section 2.3); create = `@request.auth.id != ""`; update/delete = `user.id = @request.auth.id`. Confirm contract tests from 1.1 pass.
