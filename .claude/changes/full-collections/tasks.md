...
- [x] **13.1** Write unit tests for `RegisterModule` reverted to email/password/passwordConfirm only: initial state has no `name` field; `callApi` posts `users.create` without a `name` key and returns `true` on success; a create failure is surfaced via `.error` and `callApi` returns `false`. (Authentication itself remains `LoginModule`'s responsibility, composed separately by the register page, per `design.md` 5.7 — unchanged from the prior shape.)
- [x] **13.2** Update `RegisterModule` and `RegisterForm` per 13.1 — drop the `name` input/state (maps to no field on `users`); register page navigates to `/user` after login (unchanged).
- [x] **13.3** Write unit tests for `CobrandAdminRepository.getByUserId` and `CobrandService.getAdminRecordForUserId` (integration test against the real dev PocketBase for the repository; mocked-repository unit test for the service). Also fixed a pre-existing `ICobrandAdminRepository` mock in `cobrand-service.test.ts` that broke once `getByUserId` became a required interface member.
- [x] **13.4** Add `CobrandAdminRepository.getByUserId(userId): Promise<ICobrandAdminModel | null>` and `CobrandService.getAdminRecordForUserId(userId): Promise<ICobrandAdminModel | null>`, following the exact shape of `PersonRepository.getByUserId` / `PersonService.getPersonByUserId`.
- [x] **13.5** Write unit tests for the `/user/+page.ts` three-way routing load function: linked person → `/user/properties/list`; no person but a linked cobrandAdmins record → `/user/cobrands/`; neither → `/user/setup`; either lookup rejecting → the load function's promise rejects with that error (no redirect swallows it). Established a new test pattern for `+page.ts` load functions via `vi.mock` on `getBackendClient`/`PersonService`/`CobrandService`, since none existed before. Test file is named `page.test.ts` (not `+page.test.ts`) — SvelteKit reserves the `+` filename prefix and warns/misbehaves if a non-route file uses it.
- [x] **13.6** Implement the `/user/+page.ts` three-way routing per 13.5.
- [x] **13.7** Write unit tests for `AccountSetupModule`: resolves person/cobrand-admin lookups and exposes `showChoiceAsync: Promise<boolean>`; calls the injected `redirectAction` callback with the right path (and skips the cobrand lookup entirely) when a person is linked; calls it with the cobrands path when only a cobrandAdmins record is linked; resolves `true` with no redirect when neither exists.
- [x] **13.8** Implement `AccountSetupModule` (`src/lib/modules/user/`) and the `/user/setup/` route: two choices linking to `/user/persons/setup/` and `/user/cobrands/add/`; redirect-away guard per 13.7, wired to `goto` via the same callback-injection pattern used by `PersonAddModule`/`CobrandAddModule` (modules never import `$app/navigation` directly).
- [x] **13.9** Write unit tests for `PersonSetupModule`: creates a person linked to the authenticated user via `PersonService.createPerson(name, dob, userId)`, exposes validation/creation errors via a reactive `.error` getter (not `alert()`, unlike the sibling `PersonAddModule`), and calls the injected back-action only on success.
- [x] **13.10** Implement `PersonSetupModule` (`src/lib/modules/person/`) and the `/user/persons/setup/` route: reuses the existing `PersonEditor` component; on success navigates to `/user/properties/list`. The "already has a linked person → redirect" guard lives in the route's own `+page.ts` load function (same pattern as 13.5/13.6), not in the module itself, keeping `PersonSetupModule` scoped to creation. Also extended `PersonRepository.create`/`PersonService.createPerson` with an optional trailing `userId` parameter (backward compatible — `PersonAddModule`'s unlinked-person flow still calls it with two args).
- [x] **13.11** Write E2E tests in new `tests/onboarding.test.ts`: (a) register → lands on the account setup choice page; (b) choose person → person setup → property list; (c) choose cobrand → existing cobrand-add flow → cobrand detail shows the pending-approval notice; (d) a user with a linked person visiting `/user`, `/user/setup`, or `/user/persons/setup` always lands on the property list; (e) a user with only a linked cobrandAdmins record visiting `/user` or `/user/setup` lands on `/user/cobrands/`. Fixed regressions this surfaced in pre-existing E2E fixtures: `persons.test.ts`/`cobrands.test.ts`/`agents.test.ts`/`items.test.ts` all assumed login unconditionally lands on `/user/properties/list`, which is no longer true for a user with neither a person nor a cobrandAdmins record — relaxed their `beforeEach` wait to accept any of the three post-login destinations. Also found that `agents.test.ts`'s `ensureTestCobrand` and `cobrands.test.ts`'s "add admin to cobrand" test created their cobrandAdmins fixture without `approved: true`, which Phase 12's approval gate correctly rejected once actually exercised end-to-end (agent registration and admin invitation both require an approved inviter) — added `approved: true` to both. Note: Playwright cannot run on OpenBSD at all (`playwright-core` throws "Unsupported platform: openbsd" before even attempting to launch a browser), so `npm run test:integration` had apparently never actually executed on this server for any phase; per explicit user direction this run (and only this run) executed locally on Windows against the OpenBSD server's dev backend/frontend addresses instead of on the server itself.

### Post-13.11 bugfix — Date of Birth input silently corrupted/rejected non-ISO dates

While manually verifying the onboarding flow, found that the "Date of Birth" field (`PersonEditor.svelte`, a plain free-text `TextInput` shared by the add/setup/edit person pages) let users type any text. A non-ISO format like `06/15/1990` either round-tripped to PocketBase as a raw string and got rejected with a confusing `DOB: Missing required value` (PocketBase's date parser treats an unparseable date as "missing", not "invalid format"), or — worse — got silently misparsed into a *different, wrong* date with no error at all. Reproduced and confirmed both failure modes directly against the dev server before fixing.

Fix (user chose "native date picker" over a format-validation-only approach): swapped the `TextInput` for carbon-components-svelte's `DatePicker`/`DatePickerInput` (`datePickerType="single"`, `dateFormat="Y-m-d"`), with the underlying input additionally marked `readonly` — flatpickr's calendar remains fully clickable, but direct keyboard typing (which is what let malformed dates through in the first place) is blocked outright, forcing every DOB value through the calendar so it's always a valid, unambiguous ISO date. Verified via direct reproduction that a calendar-picked date now always stores correctly (e.g. `1990-06-15 00:00:00.000Z`) and that the read-only input makes `.fill()`-based typing impossible (Playwright itself refuses to fill a non-editable element).

Updated `tests/persons.test.ts` ('create person') and `tests/onboarding.test.ts` ('choosing person setup...') to select dates via a new `pickDate()` calendar-navigation helper (sets the flatpickr year input, clicks prev/next month to the target month, clicks the target day) instead of `.fill()`, since the field is no longer directly fillable. `persons.test.ts`'s 'edit person and view roles' test needed no change — it doesn't touch DOB, and incidentally verified the DatePicker correctly displays a pre-existing DOB value loaded from the database. No unit-test changes needed (no component-level test infrastructure exists in this codebase; `PersonEditor.svelte` was already untested at that layer, consistent with every other `.svelte` component here).
